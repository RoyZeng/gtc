    ...
    private function parseTemplateFile($templateFile) {
        ...
        if(''==$templateFile) {
            ...
            if(!file_exists_case($templateFile) && C('DEFAULT_THEME') && C('BASIC_THEME')) {
                ...
                $mall_default_theme = C('BASIC_THEME');
                $group = defined('GROUP_NAME') ? GROUP_NAME . '/' : '';
                $mall_theme_path = TMPL_PATH . $group . $mall_default_theme . '/';
                $templateFile = $mall_theme_path.MODULE_NAME.(defined('GROUP_NAME')?C('TMPL_FILE_DEPR'):'/').ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX');
            }

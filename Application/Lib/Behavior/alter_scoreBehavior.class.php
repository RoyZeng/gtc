...
class alter_scoreBehavior extends Behavior {
    ...
    private function _alter_score($_data) {
        $score = C('web_score_rule.'.$_data['action']); //获取积分变量
        if (intval($score) == 0) return false; //积分为0
        if ($this->_check_num($_data['user_id'], $_data['action'])) {
            $score_data = array('score'=>array('exp','score+'.$score), 'score_level'=>array('exp', 'score_level+'.abs($score)));
            M('user')->where(array('user_id'=>$_data['user_id']))->setField($score_data); //改变用户积分
            //积分日志
    ...
    private function _check_num($uid, $action){
        ...
        //登陆次数限制
        $max_num = C('web_score_rule.'.$action.'_nums');
        //先检查统计信息
        $stat = $user_stat_mod->field('num,last_time')->where(array('user_id'=>$uid, 'action'=>$action))->find();
        if (!$stat) {
            $user_stat_mod->create(array('user_id'=>$uid, 'action'=>$action));
            $user_stat_mod->add();
        }
        $new_num = $stat['num'] + 1;
        if ($max_num == 0) {
        ...
        $user_stat_mod->where(array('user_id'=>$uid, 'action'=>$action))->save();
        return $return;
    }
